 ...
 * @author greg
 */
public class PostgreSqlAuthDB extends PostgreSqlDB implements SqlDBIface, Adapter {
    ...
    private void putToken(String tableName, String key, Token token) throws KeyValueDBException {
        try (Connection conn = getConnection()) {
            ...
            String query = 
                    "insert into ?? (token, uid, issuer, payload, tstamp, eoflife) values (?,?,?,?,?,?) "
                    +"on conflict (token) do update set uid=EXCLUDED.uid, issuer=EXCLUDED.issuer, "
                    +"payload=EXCLUDED.payload, tstamp=EXCLUDED.tstamp, eoflife=EXCLUDED.eoflife";
