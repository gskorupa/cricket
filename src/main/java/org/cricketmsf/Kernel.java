...
public abstract class Kernel {
    ...
    private ProceduresIface proceduresDictionary = new Procedures();
    ...
    private HashMap<String, String> eventHookRouterMethods = new HashMap<>();
    ...
    private Object authorizationFilter = null;
    private String authorizationFilterName = "";
    ...
    private void addHookMethodName(String key, String hookMethodName) {
        eventHookMethods.put(key, hookMethodName);
    }

    private void addHookRouterMethodName(String key, String hookMethodName) {
        eventHookRouterMethods.put(key, hookMethodName);
    }
    ...
    private void getEventHooks() {
        ...
        String procedureDictionaryName = (String) getProperties().getOrDefault("procedures", "org.cricketmsf.event.Procedures");
        try {
            proceduresDictionary = (ProceduresIface) Class.forName(procedureDictionaryName).getDeclaredConstructor().newInstance();
            LOGGER.info("Procedures dictionary class: {}", procedureDictionaryName);
        ...
        if (null != eventRouter) {
            ...
            LOGGER.info("Event router not declared");
        }
        ...
        for (Method m : this.getClass().getMethods()) {
            ...
            for (EventHook hook : portArray) {
                eventClass = hook.className();
                procedure = hook.procedure();
                addHookMethodName(getProceduresDictionary().getName(procedure) + "@" + eventClass, m.getName());
                LOGGER.info("{}::{} => {}", eventClass, getProceduresDictionary().getName(procedure), m.getName());
            }
        }
    ...
    private String getHookMethodName(String className, int procedure) {
        String result;
    ...
    private String getHookRouterMethodName(String className, int procedure) {
        String result;
    ...
    private synchronized void loadAdapters(Configuration config) throws Exception {
        ...
        setAuthorizationFilter(config.getProperty("filter"));
    ...
    private void setAuthorizationFilter(String filterName) {
        if (null == filterName || filterName.isEmpty()) {
            authorizationFilterName = DEFAULT_AUTHORIZATION_FILTER;
        } else {
            authorizationFilterName = filterName;
        }
        try {
            Class c = Class.forName(authorizationFilterName);
            authorizationFilter = c.newInstance();
    ...
    public Filter getAuthorizationFilter() {
        if (null == authorizationFilter) {
            try {
                Class c = Class.forName(authorizationFilterName);
                authorizationFilter = (Filter) c.newInstance();
                authorizationFilter = c.newInstance();
            } catch (ClassCastException | ClassNotFoundException | InstantiationException | IllegalAccessException e) {
                System.out.println(e.getMessage());
                //e.printStackTrace();
        ...
        return (Filter) authorizationFilter;
    }
    ...
    public void setCorsHeaders(String corsHeaders) {
        ...
        if (null == tmpCors || tmpCors.isBlank()) {
            tmpCors = "Access-Control-Allow-Origin:*|Access-Control-Allow-Credentials:true|Access-Control-Allow-Methods: POST, GET, OPTIONS, DELETE, PUT|Access-Control-Allow-Headers: Authorization, Origin, X-Requested-With, Content-Type, Accept, Accept-Language, Content-Language|Access-Control-Max-Age: 1728000";
        }
    ...
    /**
     * @return the proceduresDictionary
     */
