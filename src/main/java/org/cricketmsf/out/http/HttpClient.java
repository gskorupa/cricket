        ...
        if (transform) {
            switch (request.properties.get("Content-Type")) {
                ...
                case X_WWW_FORM:
                    ...
                    break;
                default:
                    throw new AdapterException(AdapterException.UNSUPPORTED_CONTENT_TYPE, "unsupported content type: " + request.properties.get("Content-Type"));
            }
        } else {
        ...
        try {
            ...
            if (urlObj.getProtocol().equalsIgnoreCase("HTTPS")) {
                ...
                headers.keySet().forEach(key -> {
                    result.setHeader(key, headers.get(key));
                });
                if (isRequestSuccessful(result.getCode())) {
                    ...
                    //result.setContent("");
                    throw new AdapterException(AdapterException.STRANGE_CODE, "received code "+result.getCode());
                }
            } else {
            ...
            */
            ...
            throw new AdapterException(AdapterException.CRYPTOGRAPHY_EXCEPTION, e.getMessage());
            /*
            String message = e.getMessage();
            result.setCode(426);
            result.setMessage(message);
